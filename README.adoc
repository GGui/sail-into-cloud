...
oc expose svc helloworld-tracer -n demo-apps <3>
----
...
<3> Expose the helloworld-tracer service route
...
[code,sh]
----
minishift openshift service helloworld-tracer -n demo-apps --url <1>
curl $(minishift openshift service helloworld-tracer -n demo-apps --url)/hellotracer <2>
----

<1> Get the url to access the application, and append the `/hellotracer` to url 
<2> Quick invocation via curl

e.g. the hello-world-tracer the URL will look like __http://helloworld-tracer-demo-apps.192.168.64.19.nip.io/hellotracer__
...
If you run a load test with any load test tools like jmeter, gatling etc., to the url `minishift openshift service hola -n demo-apps --url` to see the routing rules getting applied.
...
[code,sh]
----
./mvnw -Dskip.gatlingTests=false -Dgatling.simulationClass=HolaServiceSimulation -Dhola.it.endpoint.url=http://hola-demo-apps.192.168.64.19.nip.io clean integration-test
----
...
[code,sh]
----
./mvnw -Dskip.gatlingTests=false -Dgatling.simulationClass=HolaCanaryPremiumSimulation -Dhola.it.endpoint.url=http://hola-demo-apps.192.168.64.19.nip.io clean integration-test
----
